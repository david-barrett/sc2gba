...
int FireMmrnmhrm(pPlayer pl)
{
	...
	if (pl->form==0)
	{
		//probably need to increse in.object.size!!
		//laser
		...
		pl->weapon[b].object.size=(b==0?32:16);
	...
	if (b>=0)
	{
		...
		//pl->weapon[b].object.angle = ModifyAngle(pl->object.angle,(i==0?-30:+30));
		if (i==0)
		{
			pl->weapon[b].object.actualangle = pl->object.actualangle-1;
			if (pl->weapon[b].object.actualangle==-1)
				pl->weapon[b].object.actualangle=15;
		}
		else
		{
			pl->weapon[b].object.actualangle = pl->object.actualangle+1;
					if (pl->weapon[b].object.actualangle==16)
			pl->weapon[b].object.actualangle=0;
		}

		pl->weapon[b].object.angle=(pl->weapon[b].object.actualangle*45)>>1;

		pl->weapon[b].object.xspeed = ((MISSILE_SPEED) * (s32)SIN[pl->weapon[b].object.angle])>>8;
		pl->weapon[b].object.yspeed = ((MISSILE_SPEED) * (s32)COS[pl->weapon[b].object.angle])>>8;
...
int aiMmrnmhrm(pPlayer ai, pObject ObjectsOfConcern, COUNT ConcernCounter)
{
			...
			&& !(ai->ship_input_state & WEAPON))
	{
		...
		s16 directionangle=ai->object.angle;
		s16 travelangle=FindAngle(0,0,ai->object.xspeed,ai->object.yspeed);
		...
		if (ai->form==0)//in slow form
		{
			if (lpEvalDesc->which_turn > 8)
			{
				if (opp->ManeuverabilityIndex <= SLOW_SHIP
						|| TurnAngle(directionangle,travelangle,90)==0)
					ai->ship_input_state |= SPECIAL;
			}
		}
		else
		{
			...
			if (lpEvalDesc->which_turn < 16)
			{
				if (lpEvalDesc->which_turn <= 8
					...
					&&	TurnAngle(directionangle,travelangle,180)!=0)
				ai->ship_input_state |= SPECIAL;
		}
	}
