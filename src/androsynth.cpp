...
int FireAndrosynth(pPlayer pl)
{
	...
	if (b>=0)
	{
		...
		pl->weapon[b].object.xpos = pl->object.xpos+((s32)(pl->offset*2 * SIN[pl->object.angle])>>8);
		pl->weapon[b].object.ypos = pl->object.ypos-((s32)(pl->offset*2 * COS[pl->object.angle])>>8);
...
int SpecialAndrosynth(pPlayer pl)
{
	...
	pl->blaze=1;
	pl->object.mass_points=BLAZER_MASS;
				...
				&& lpEvalDesc->parent)
		{
			...
			.object.size delta_x, delta_y,
				...
				GetCurrentVelocityComponents (&ShipPtr->velocity,
						&ship_delta_x, &ship_delta_y);
				GetCurrentVelocityComponents (&lpEvalDesc->tr->velocity,
						&other_delta_x, &other_delta_y);
...
void PostAndro(pPlayer pl)
{
	...
	if (pl->blaze==1)
	{
		...
		else
		{
			...
			pl->object.xpos+=((BLAZER_THRUST) * (s32)SIN[pl->object.angle])>>8;
			pl->object.ypos-=((BLAZER_THRUST) * (s32)COS[pl->object.angle])>>8;
