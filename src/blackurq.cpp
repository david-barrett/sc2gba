...
int SpecialBlackurq(pPlayer pl)
{
	...
	for (int b=4;b<12;b++)
		{
			pl->weapon[b].type=SIMPLE;
			pl->weapon[b].object.life=GAS_LIFE;
			...
			pl->weapon[b].object.size=16;
			pl->weapon[b].object.angle = (b-4)*45;

			pl->weapon[b].object.xspeed=((s32)(GAS_SPEED * SIN[pl->weapon[b].object.angle])>>8);
			pl->weapon[b].object.yspeed=((s32)(GAS_SPEED * COS[pl->weapon[b].object.angle])>>8);
	...
	#define FRAGMENT_RANGE (FRAGMENT_LIFE * FRAGMENT_SPEED)
			...
			for (; h; h = hNext)
			{
				...
				if (!(BuzzSawPtr->state_flags & NONSOLID)
						...
						&& BuzzSawPtr->object.life_span > MISSILE_LIFE * 3 / 4
				...
				{
					{
						...
						if (!PlotIntercept (BuzzSawPtr,
								lpEvalDesc->tr, BuzzSawPtr->object.life_span,
								FRAGMENT_RANGE / 2))
...
void MoveBlades(pWeapon w)
{
	...
	if (w->turn_wait==0)
	{
		d=distanceBetweenPoints(target->object.xpos,target->object.ypos,w->object.xpos,w->object.ypos);
	if (target->crew>0&&d<80)
	{
		w->turn_wait=BLADE_WAIT;
		s16 angle = FindAngle(w->object.xpos,w->object.ypos,target->object.xpos,target->object.ypos);

		w->object.xspeed+=((BLADE_SPEED) * (s32)SIN[angle])>>8;
		w->object.yspeed+=((BLADE_SPEED) * (s32)COS[angle])>>8;
	}
	else
	{
		w->object.xspeed=0;
...
void PostBlackurq(pPlayer p)
{
	...
	for (int i=0;i<4;i++)
	{
		...
		{
		p->weapon[i].object.actualangle+=1;
		if (p->weapon[i].object.actualangle==16)
			p->weapon[i].object.actualangle=0;
		p->weapon[i].object.angle=(p->weapon[i].object.actualangle*45)>>1;
		if (p->weapon[i].object.life==MISSILE_LIFE-1)
			p->weapon[i].object.life++;
		}
	}
	...
	{
		p->weapon[p->currentweapon].object.xspeed=0;
		p->weapon[p->currentweapon].object.yspeed=0;
		p->charging=0;
		if (v3do)
		{
